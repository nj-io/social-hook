...
|---------|----------|-------|
| `quickstart` | ok | Enriched: explains full onboarding flow; `--strategy`, `--branch` flags documented |
...
| `credentials *` | ok | Group help enriched; `add --set` for non-interactive agent/CI use |
| `cycles *` | ok | Group help enriched; `show` displays per-strategy outcomes with topic names |
...
| `decision *` | ok | `list` and `delete` enriched with decision context; new `batch-evaluate` command |
...
| quickstart.md | complete | Preview draft → promote flow still valid; `--strategy`/`--branch` flags added; targets onboarding in backlog |

## Recurring Checks

| Check | Last passed | Notes |
|-------|-------------|-------|
| CLI docs are fresh (`generate_cli_docs.py` output matches committed) | 2026-04-07 | |
| `mkdocs.yml` nav entries match files in `site-docs/cli/` | 2026-04-07 | |
| OAuth env vars in config.md use OAuth 2.0 names (`X_CLIENT_ID`, not `CONSUMER_KEY`) | 2026-04-07 | |
| `ruff check src/ tests/` passes | 2026-04-07 | |
| `mypy src/social_hook/` has no new errors (only pre-existing library stub issues) | 2026-04-07 | 28 errors, all `import-untyped` or pre-existing |
| All CLI commands with poor/partial docstrings have been enriched | 2026-04-07 | 16 commands across 6 files enriched |
| `pipeline.md` accurately describes the two-stage evaluation flow and targets path | 2026-04-07 | |
| `narrative-arcs.md` uses `episode_tags` (not `episode_type`) and documents strategy-scoped arcs | 2026-04-07 | |
| All interactive CLI commands (e.g., `credentials add`) have documented non-interactive equivalents for agent/CI use | 2026-04-07 | `credentials add --set`, all destructive commands have `--yes`; `setup` wizard → write config.yaml directly |
