...
|-------|-------------|-------|
| CLI docs are fresh (`generate_cli_docs.py` output matches committed) | 2026-06-10 | Fixed `generate_cli_docs.py` for typer 0.26.7 compatibility (TyperOption/TyperArgument class hierarchy) |
| `mkdocs.yml` nav entries match files in `site-docs/cli/` | 2026-06-10 | |
| OAuth env vars in config.md use OAuth 2.0 names (`X_CLIENT_ID`, not `CONSUMER_KEY`) | 2026-06-10 | |
| `ruff check src/ tests/` passes | 2026-06-10 | |
| `mypy src/social_hook/` has no new errors (only pre-existing library stub issues) | 2026-06-10 | 28 errors, all `import-untyped` pre-existing |
| All CLI commands with poor/partial docstrings have been enriched | 2026-06-10 | |
| `pipeline.md` accurately describes the two-stage evaluation flow and targets path | 2026-06-10 | |
| `narrative-arcs.md` uses `episode_tags` (not `episode_type`) and documents strategy-scoped arcs | 2026-06-10 | |
| All interactive CLI commands (e.g., `credentials add`) have documented non-interactive equivalents for agent/CI use | 2026-06-10 | `credentials add --set`, all destructive commands have `--yes`; `setup` wizard → write config.yaml directly |
| All CLI command groups have enriched group-level help text (not just terse labels) | 2026-06-10 | 10 groups re-enriched this run (arc, bot, config, decision, draft, inspect, logs, manual, media, memory, snapshot); all 22 groups now descriptive |
