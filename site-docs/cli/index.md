...
| [`arc`](arc.md) | Manage narrative arcs — multi-post storylines that thread through your content and give the evaluator continuity context. |
| [`bot`](bot.md) | Start, stop, and check the bot daemon that runs the scheduler and posts approved drafts on their schedule. |
| [`brief`](brief.md) | View and edit the project brief used by the evaluator and drafter. |
| [`config`](config.md) | View and modify config.yaml — show the full config, get a single value by dotted key path, or set a value. |
...
| [`decision`](decision.md) | View and manage evaluator decisions — the per-commit verdicts on whether to draft, skip, or hold content. |
| [`draft`](draft.md) | Manage the full draft lifecycle — approve, reject, schedule, edit, redraft, promote, and post content generated by the evaluator. |
| [`inspect`](inspect.md) | Inspect system state: view the decision log, pending drafts, LLM token usage, and configured platforms. |
...
| [`manual`](manual.md) | Run pipeline steps by hand — evaluate a commit, create drafts from a decision, consolidate decisions, or post a draft directly. |
| [`media`](media.md) | Housekeeping for generated media files — clean up orphaned images from the media cache. |
| [`memory`](memory.md) | Manage voice memories — per-project feedback records that train the drafter to match your preferred tone and style. |
...
| [`snapshot`](snapshot.md) | Save, restore, and reset the SQLite database — useful for testing, backups, and recovering from mistakes. |
