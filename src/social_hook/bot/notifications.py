...
def format_draft_review(
    ...
    Args:
        ...
        part_count: Number of parts in thread
        ...
        vehicle: Content vehicle type (single/thread/article)
    ...
    # Vehicle in platform line: [X · single]
    platform_display = f"{platform} \u00b7 {vehicle or 'single'}"
    ...
    if is_thread and part_count:
        lines.append(f"Thread: {part_count} parts")
    ...
    # Articles: truncate to 500 chars + view in dashboard
    is_article = vehicle == "article"
    display_content = content
    if is_article and len(content) > 500:
        display_content = content[:500] + "\n... view full in dashboard"
...
def get_review_buttons_normalized(
    draft_id: str, platform: str = "", is_intro: bool = False, preview_mode: bool = False
) -> list:
    """Get review buttons as normalized ButtonRow list.

    Args:
        draft_id: Draft ID for callback data
        platform: Platform name (for display, not gating)
        is_intro: Whether this draft is an introduction post
        preview_mode: When True, shows Connect Account instead of
