...
bot_app = typer.Typer(
    name="bot",
    help="Start, stop, and check the bot daemon that runs the scheduler and posts approved drafts on their schedule.",
    no_args_is_help=True,
)
app.add_typer(
    bot_app,
...
app.add_typer(
    inspect_app,
    name="inspect",
    help="Inspect system state: view the decision log, pending drafts, LLM token usage, and configured platforms.",
)
...
app.add_typer(
    memory_app,
    name="memory",
    help="Manage voice memories — per-project feedback records that train the drafter to match your preferred tone and style.",
)
...
app.add_typer(
    arc_app,
    name="arc",
    help="Manage narrative arcs — multi-post storylines that thread through your content and give the evaluator continuity context.",
)
...
app.add_typer(
    decision_app,
    name="decision",
    help="View and manage evaluator decisions — the per-commit verdicts on whether to draft, skip, or hold content.",
)
...
app.add_typer(
    draft_app,
    name="draft",
    help="Manage the full draft lifecycle — approve, reject, schedule, edit, redraft, promote, and post content generated by the evaluator.",
)
...
app.add_typer(
    media_app,
    name="media",
    help="Housekeeping for generated media files — clean up orphaned images from the media cache.",
)
